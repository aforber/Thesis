...
$\qquad$ The misuse of opioids has become an increasing problem in the United States, and the need to identify patients at increase risk for opioid abuse is immediate. STATS ON OPIOID USE. 
...
$\qquad$ However this presents a statical problem, as this is a rare outcome which makes prediction more challenging. Predictive modeling with imbalanced data has been found to report low sensitivity (reference?). Many people have tried to combat this issue in the past. GIVE EXAMPLES. 
...
$\qquad$ The first approach used to improve performance was to choose a more informed probability cut-point for the data. This was done using the Youden Index, which finds the maximum of the receiver operating characteristic (ROC) curve (reference here!) with the pROC package. 
